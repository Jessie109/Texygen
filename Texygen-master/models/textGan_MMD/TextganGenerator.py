...
###################################### temp sweep ######################################
        ...
        gen_x_temp = tensor_array_ops.TensorArray(dtype=tf.int32, size=self.sequence_length,
                                             dynamic_size=False, infer_shape=True)
        gen_ot_temp = tensor_array_ops.TensorArray(dtype=tf.float32, size=self.sequence_length,
                                             dynamic_size=False, infer_shape=True)

        def _g_recurrence_temp(i, x_t, h_tm1, gen_o_temp, gen_x_temp, gen_ot_temp, alpha=self.alpha):
            h_t = self.g_recurrent_unit(x_t, h_tm1)  # hidden_memory_tuple
            ...
            gen_ot_temp = gen_ot_temp.write(i, x_tp1)
            return i + 1, x_tp1, h_t, gen_o_temp, gen_x_temp, gen_ot_temp, alpha

        _, _, _, self.gen_o_temp, self.gen_x_temp, self.gen_ot_temp, self.alpha = control_flow_ops.while_loop(
            cond=lambda i, _1, _2, _3, _4, _5, _6: i < self.sequence_length,
            body=_g_recurrence_temp,
            loop_vars=(tf.constant(0, dtype=tf.int32),
                       tf.nn.embedding_lookup(self.g_embeddings, self.start_token), self.h0, gen_o, gen_x, gen_ot, self.alpha))
        ...
        self.gen_ot_temp = self.gen_ot_temp.stack()
        self.gen_ot_temp = tf.slice(self.gen_ot_temp,begin=[0,0,0],size=[sequence_length, batch_size, emb_dim])
        self.gen_ot_temp = tf.transpose(self.gen_ot_temp, perm=[1, 0, 2])  # batch_size x seq_length x g_emb_dim
    ...
    def generate_temp(self, sess, temp, get_z = False):
        self.alpha = temp
        z_h0 = np.random.uniform(low=-.01, high=1, size=[self.batch_size, self.emb_dim])
        z_c0 = np.zeros(shape=[self.batch_size, self.emb_dim])
        feed = {
        ...
        outputs = sess.run(self.gen_x_temp, feed)
        if not get_z:
            return outputs
        return outputs, z_h0

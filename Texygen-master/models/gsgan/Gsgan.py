...
class Gsgan(Gan):
    def __init__(self, oracle=None):
        ...
        self.name = 'gsgan'
        self.generate_temp_dataset = 'coco'

        self.oracle_file = 'save/gsgan/oracle.txt'
        self.generator_file = 'save/gsgan/generator.txt'
        self.test_file = 'save/gsgan/test_file.txt'
    ...
    def evaluate(self):
        ...
        if self.log is not None:
            if self.epoch == 0 or self.epoch == 1:
                ...
                self.log.write('\n')
            scores = super(Gsgan, self).evaluate()
            for score in scores:
                self.log.write(str(score) + ',')
            self.log.write('\n')
            return scores
        return super(Gsgan, self).evaluate()
...
#         self.add_metric(docsim)
    ...
    def train_real(self, size,itr, data_loc=None):
        ...
        for alpha in self.temps:
            gen_file= 'gen_data/gsgan/coco/'+str(itr)+'_'+str(size)+'_'+str(alpha)+'.txt'

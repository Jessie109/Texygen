...
################################## temp sweep ##########################################
        ...
        def _g_recurrence(i, x_t, h_tm1, gen_o_temp, gen_x_temp, gen_ot_temp):
            h_t = self.g_recurrent_unit(x_t, h_tm1)  # hidden_memory_tuple
            h_t = self.add_gumbel(h_t)
            o_t = self.g_output_unit(h_t)  # batch x vocab , logits not prob
            x_tp1 = tf.nn.softmax(o_t / self.alpha)
            next_token = tf.cast(tf.argmax(o_t, axis=1), tf.int32)
            ...
            return i + 1, x_tp1, h_t, gen_o_temp, gen_x_temp, gen_ot_temp

        _, _, h_t, self.gen_o_temp, self.gen_x_temp, self.gen_ot_temp = control_flow_ops.while_loop(
            cond=lambda i, _1, _2, _3, _4, _5: i < self.sequence_length,
            body=_g_recurrence,
            loop_vars=(tf.constant(0, dtype=tf.int32),
                       tf.nn.embedding_lookup(onehot, self.start_token), self.h0, gen_o_temp, gen_x_temp, gen_ot_temp))
    ...
    def generate_temp(self, sess, temp):
        self.alpha = temp
        z_h0 = np.random.uniform(low=-1, high=1, size=[self.batch_size, self.hidden_dim])
        z_c0 = np.random.uniform(low=-1, high=1, size=[self.batch_size, self.hidden_dim])
        feed = {

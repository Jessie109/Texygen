...
class Leakgan(Gan):
    def __init__(self, oracle=None):
        ...
        self.oracle_file = 'save/leakgan/oracle.txt'
        self.generator_file = 'save/leakgan/generator.txt'
        self.test_file = 'save/leakgan/test_file.txt'
    ...
    def evaluate(self):
        ...
        if self.log is not None:
            if self.epoch == 0 or self.epoch == 1:
                ...
                self.log.write('\n')
            scores = super(Leakgan, self).evaluate()
            for score in scores:
                self.log.write(str(score) + ',')
            self.log.write('\n')
            return scores
        return super(Leakgan, self).evaluate()
    ...
    def train_real(self, size,itr, data_loc=None):
        ...
        for epoch in range(self.adversarial_epoch_num//10):
            ...
            for alpha in self.temps:
                gen_file = 'gen_data/leakgan/coco/' + str(itr) + '_' + str(size) + '_' + str(alpha) + '.txt'
                #                     gen_file= 'gen_data/seqgan/coco/'+'ds.txt'
                print('alpha', alpha, gen_file)
                generate_samples_temp(self.sess, self.generator, self.batch_size, self.generate_num, alpha,
                                      self.generator_file)
